...
最後一行必需能在螢幕顯現此值。
```{r ans11}
length(unique(transcriptData$學號))
```
...
transcriptData %>%
  ...
  ) -> transcriptDataWithSemesterCounts1
...
transcriptDataWithSemesterCounts1%>%
  ...
  mutate(
    最大學期=max(第幾學期))%>%
  ungroup()%>%
  filter(最大學期>=8,第幾學期<=8)->
  filteredTranscriptData1
...
transcriptData %>%
  ...
  mutate(
    第幾學期=semesterNumber(學年學期)
  ) -> transcriptDataWithSemesterCounts ## 到此，每個學生的每個學期是第幾學期就都有了

transcriptDataWithSemesterCounts %>%
  group_by(學號) %>%
  mutate(最大學期=max(第幾學期)) %>%
  ungroup %>%
  filter(最大學期>=8, 第幾學期<=8) -> 
...
filteredTranscriptData1%>%
  ...
  ungroup() -> grade4YearsNoGE1

grade4YearsNoGE1%>%
  group_by(學系,學屆)%>%
  mutate(
    學屆PR不含通識=percent_rank(學屆平均成績)*100
  )%>%
  ungroup()->grade4YearsNoGE1

grade4Years1%>%
  left_join(grade4YearsNoGE1,by=c("學號","學系","學屆"))->grade4Years1
```
...
grade4Years1%>%
  mutate(
    PR差異=abs(學屆PR-學屆PR不含通識)
  )%>%
  group_by(學系)%>%
  summarise(
    PR差異中位數=median(PR差異)
  )->PRdiffEachDepartment1
```
